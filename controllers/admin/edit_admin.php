...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username)) {
        header("Location: ../../views/admin/edit_admin.php?id=$id&error=Username is required.");
        exit();
    }
    ...
    try {
        ...
        if (!$existingAdmin) {
            header("Location: ../../controllers/admin/admin_view.php?error=Admin not found.");
            exit();
        }
        ...
        header("Location: ../../controllers/admin/admin_view.php?success=Admin updated successfully.");
        exit();
    } catch (Exception $e) {
        // Handle any errors and redirect back with error message
        header("Location: ../../views/admin/edit_admin.php?id=$id&error=" . urlencode($e->getMessage()));
        exit();
    }
} else {
    // If request is not POST, redirect to admin list
    header('Location: ../../controllers/admin/admin_view.php');
    exit();
}
