...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username)) {
        header("Location: index.php?page=admin_view&id=$id&error=Username is required.");
        exit();
    }
    ...
    try {
        ...
        header("Location: index.php?page=admin_view&success=Admin updated successfully.");
        exit();
    } catch (Exception $e) {
        // Handle any errors and redirect back with error message
        header("Location: index.php?page=admin_view&id=$id&error=" . urlencode($e->getMessage()));
        exit();
    }
} else {
